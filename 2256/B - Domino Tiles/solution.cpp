...
void solve() {
    ...
    for (int i = 0; i < n; i += 2) {
        char p1 = ((i / 2) % 2 == 0) ? '0' : '1';
        char p2 = ((i / 2) % 2 == 0) ? '1' : '0';
        if (s[i] != '?' && s[i] != p1) e1 = false;
        if (s[i] != '?' && s[i] != p2) e2 = false;
    }
    bool o1 = true, o2 = true;
    for (int i = 1; i < n; i += 2) {
        char p1 = (((i - 1) / 2) % 2 == 0) ? '0' : '1';
        char p2 = (((i - 1) / 2) % 2 == 0) ? '1' : '0';
        if (s[i] != '?' && s[i] != p1) o1 = false;
        if (s[i] != '?' && s[i] != p2) o2 = false;
    }
    ll ans = 1LL * (e1 + e2) * (o1 + o2) % MOD;
    cout << ans << endl;
}
int main() {
    AHMED;
